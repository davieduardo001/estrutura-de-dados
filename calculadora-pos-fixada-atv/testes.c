...
int escolher_operacao() {
    int escolha;
    printf("Escolha uma expressão para avaliar (de 1 a %d): ", MAX_EXPR);
    scanf("%d", &escolha);
    while (escolha < 1 || escolha > MAX_EXPR) {
        printf("Escolha inválida. Escolha uma expressão de 1 a %d: ", MAX_EXPR);
        scanf("%d", &escolha);
    }
    return escolha - 1;
}
